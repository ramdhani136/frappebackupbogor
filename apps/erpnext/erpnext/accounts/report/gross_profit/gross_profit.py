...
class GrossProfitGenerator(object):
	...
	def get_buying_amount(self, row, item_code):
		...
		if item_code in self.non_stock_items and (row.project or row.cost_center):
			...
			if row.update_stock  and my_sle:
	...
	def load_invoice_items(self):
		...
		self.si_list = frappe.db.sql(
			...
			select
				...
				`tabSales Invoice Item`.base_rate as base_net_rate, `tabSales Invoice Item`.base_net_amount as base_amount,
		...
		return frappe._dict(
			{
				...
				"base_amount": frappe.db.get_value("Sales Invoice", row.parent, "base_net_total"),
			}
		)
